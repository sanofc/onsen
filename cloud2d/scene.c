...
#define K					0.10	//Buoyancy Coefficient
#define G					0.005			//Gravity Coefficient
...
void scene(){
	...
	for(int i = X * 0.3; i < X-X*0.3; i++){
		//srand(100);
		int j = 2;
			double pv = (double)(rand()%100)/100.0;
			double pt =1;
			t[i][j] = t[i][j] + 0.10 * pt;
			v[i][j] = v[i][j] + 0.3 * pv;
	}
	...
	double b = 30;
	double c = -1.6;
	double m = MIN(a * exp(-b / ((t[i][j]-q) + c)),v[i][j]+s[i][j]);
...
//	if(s[i][j] > 0){
	...
	//double buoy  = G * s[i][j];
	double noise_x = ((double)(rand()%100)/100.0-0.5) * 0.03;
	//double noise_x = 0;
	double noise_y = ((double)(rand()%100)/100.0-0.5) * 0.03;
	//double noise_y = 0;
