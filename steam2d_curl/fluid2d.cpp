...
double g_ref(double **g, int i, int j) {
	double ref;
	if (g == p || g == t || g == t_swap) ref = g[CLIP(i, 0, X - 1)][CLIP(j, 0, Y - 1)];
...
void compute_force() {
	START_FOR_C
		if (i == 0)continue;
	u[i][j] += (f[0][i][j] + f[0][i - 1][j]) * 0.5 ;
	END_FOR
		START_FOR_C
		if (j == 0)continue;
	v[i][j] += (f[1][i][j] + f[1][i][j - 1]) * 0.5 ;
	END_FOR
}
...
void scene() {
	...
	for (int i = X * 0.3; i < X - X*0.3; i++) {
		...
		p->u[0] = 0;//interpolation(u, p->p[0] * N, p->p[1] * N, X + 1, Y);
		p->u[1] = 0;//interpolation(v, p->p[0] * N, p->p[1] * N, X, Y + 1);
		p->dens = P_DENS;
		add_particle(p);
