...
#define K					0.02	//Buoyancy Coefficient
...
void scene(){
	...
	for(int i = X * 0.3; i < X-X*0.3; i++){
		//srand(100);
		int j = 0;
		double pv = (double)(rand()%20)/100.0;
		double pt =0;
		t[i][j] = 15.0;//t[i][j] + 0.10 * pt;
		v[i][j+1] = v[i][j+1] +  pv;
	}
...
//	if(s[i][j] > 0){
	...
	//double buoy  = G * s[i][j];
	double noise_x = ((double)(rand()%100)/100.0-0.5) * 0.01;
	//double noise_x = 0;
	double noise_y = ((double)(rand()%100)/100.0-0.5) * 0.01;
	//double noise_y = 0;
