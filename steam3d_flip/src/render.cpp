...
static FLOAT interp( FLOAT *** d, int width, int height, int depth, FLOAT x, FLOAT y, FLOAT z )
{
	...
	if (x<0.0) x=0.0; if (x>width) x=width; i0=std::min(width-2,(int)x); i1=i0+1;
	if (y<0.0) y=0.0; if (y>height) y=height; j0=std::min(height-2,(int)y); j1=j0+1;
	if (z<0.0) z=0.0; if (z>depth) z=depth; k0=std::min(depth-2,(int)z); k1=k0+1;
...
void render::render( FLOAT ***d, /*FLOAT sphere_r,*/ int N, int frame ) {
	...
	// Light Position
	const FLOAT lightPos[3] = { 0.5, 1.5, -1.0 };
	...
	OPENMP_FOR FOR_EVERY_PIXEL(w) {
		...
		// Sphere Color
		unsigned char sphere_color[3] = { 255, 0, 0 };
	...
	char filename[64];
	sprintf( filename, "render_%02d.bmp", frame );
