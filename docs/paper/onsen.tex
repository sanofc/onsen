...
\begin{abstract}
本研究ではCGによる湯気のビジュアルシミュレーションを行うことを目的とする。湯気の状態に影響する水蒸気量、温度といったパラメータにより実用的な計算時間により湯気の表現を可能とする。流体計算は格子法と粒子法を組み合わせることにより相転移を表現する手法を提案する。
\end{abstract}
...
本稿においては温度、水蒸気量といった湯気の状態に影響するパラメータを変化させることによりCGによる湯気の可視化を行うことを目的とする。
流体計算は格子法と粒子法を組み合わせることにより相転移を表現する手法を提案する。
